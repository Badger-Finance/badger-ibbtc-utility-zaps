...
contract BadgerVaultZap is PausableUpgradeable {
    ...
    address public constant CURVE_IBBTC_METAPOOL =
        0xFbdCA68601f835b27790D98bbb8eC7f05FDEaA9B; // address of ibbtc crv metapool
    address public constant CURVE_IBBTC_DEPOSIT_ZAP =
        0xbba4b444FD10302251d9F5797E763b0d912286A1; // address of ibbtc crv deposit zap
    address public constant BADGER_RENWBTC_IBBTC_ZAP =
        0xaE96fF08771a109dc6650a1BdCa62F2d558E40af; // address of renBTC/wBTC -> ibbtc zap
    ...
    function deposit(uint256[] calldata _amounts) public whenNotPaused {
        uint256[] memory depositAmounts = new uint256[](4);

        for (uint256 i = 0; i < 4; i++) {
            if (_amounts[i] > 0) {
                IERC20Upgradeable(ASSETS[i]).safeTransferFrom(
                    msg.sender,
                    address(this),
                    _amounts[i]
                );
                if (i == 1 || i == 2) {
                    (
                        ...
                        uint256 bBTC,
                        uint256 fee
                    ) = IZapRenWBTC(BADGER_RENWBTC_IBBTC_ZAP).calcMint(
                            ASSETS[i],
                            _amounts[i]
                        );
                    depositAmounts[0] += IZapRenWBTC(BADGER_RENWBTC_IBBTC_ZAP)
                        .mint(IERC20(ASSETS[i]), _amounts[i], 0, i - 1, bBTC);
                } else {
